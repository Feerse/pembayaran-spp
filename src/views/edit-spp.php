...
<a href="?url=spp" class="border p-2 rounded-md inline-block mb-3 shadow-md">
    < Kembali</a>
        ...
        <form class="max-w-md mx-auto" action="../controllers/editSppController.php?id_spp=<?= $id_spp; ?>" method="post">
            ...
            <div class="relative z-0 w-full mb-5 group">
                <input value="<?= $data['nominal'] ?>" type="number" name="nominal" id="nominal" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " maxlength="13" required />
            ...
            <button type="submit" class="text-white bg-blue-600 hover:bg-blue-700 font-medium rounded-lg text-sm w-full sm:w-auto px-4 py-2 text-center duration-300">Ubah</button>
        </form>